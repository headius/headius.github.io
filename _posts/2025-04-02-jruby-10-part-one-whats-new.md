...
I am very excited to introduce you to [JRuby](https://github.com/jruby/jruby/) 10, the biggest leap forward since "JRuby 9000" was released almost a decade ago.

With up-to-date Ruby compatibility, support for modern JVM features, and a big cleanup of internal code and external APIs, we believe this is our most important release ever. This article will provide first look at these improvements and help you get started on your JRuby 10 journey.
...
That meant implementing [Ruby 3.2](https://github.com/jruby/jruby/issues/7517), [3.3](https://github.com/jruby/jruby/issues/8029), and [3.4](https://github.com/jruby/jruby/issues/8395) features, and getting literally thousands of new tests and specs passing.
...
Like many projects in the Java world, we chose to maintain support for Java 8 for over a decade after it was released, due to challenging issues involving Java 9+ and the new restrictions of the Java Platform Module System. Even though we made sure JRuby supported the latest Java releases, we were limited to Java 8-level features even while the platform added amazing enhancements like native function calls ([Project Panama](https://openjdk.org/projects/panama/)) and lightweight threads ([Project Loom](https://openjdk.org/projects/loom/)).

For JRuby 10, we're finally updating our minimum Java version... to the most recent "long term support" version 21. Many of the other JRuby features described in this post are enabled because of that move.

We've already started to integrate everything that [Java 21](https://openjdk.org/projects/jdk/21/) has to offer, and we're looking forward to bringing ten years of JVM enhancements to Ruby users.
...
JRuby 10 runs with [full invokedynamic optimization](https://github.com/jruby/jruby/pull/8450) by default. That means you'll get the best current performance for your Ruby scripts and applications without passing any additional flags. For development and testing environments, where aggressive optimizations are usually not helpful and just slow down command-line use, we provide other options...

### Startup time improvements

The number one complaint from JRuby users has always been startup time (and to a lesser extent, warmup time of large apps). With the leap to Java 21, we've incorporated two new features to help JRuby users get their code running more quickly:
...
* [Project Leyden](https://openjdk.org/projects/leyden/) is the next-generation "AppCDS", also storing data like JIT-compiled native code and optimization profiles from previous runs. The goal of Leyden is to eventually save off everything needed to start right up with optimized code, skipping the slow early stages of execution. The JRuby Team will incorporate Leyden flags into our launcher as they become available... and JRuby users will just have to upgrade their JDK to take advantage.

These features combined with our reduced-overhead `--dev` flag mean JRuby starts up faster than ever before... fast enough to take the pain out of command-line development with standard Ruby tools.
...
Most Rubyists will be familiar with Ruby managers and switchers like [rbenv](https://github.com/rbenv/rbenv), [rvm](https://rvm.io/), or [chruby](https://github.com/postmodern/chruby). JRuby 10 preview snapshots can be installed today as "jruby-head" or "jruby-dev", and after our release as "jruby-10" or just "jruby".

But JRuby is also a JVM-based project (write once, run anywhere, remember?), so [installing it yourself](https://www.jruby.org/) is as easy as unpacking a tarball and putting the `bin` dir in your `PATH`. There's no build step and no build tools needed to start using JRuby today.
...
Keeping up with Rails is no small feat, and with only a handful of contributors we have tended to lag a bit behind. JRuby currently has database support for ActiveRecord up to 7.1, and work is going on now to support Rails 8 and beyond. If you have an interest in helping the JRuby team update our ActiveRecord adapter ([ActiveRecord-JDBC](https://github.com/jruby/activerecord-jdbc-adapter), which uses the pure-JVM Java DataBase Connectivity API), please let us know! We're hoping to have full support for Rails 8 by this summer.

### Generating an app

The typical Rails commands all work on JRuby exactly the same as on standard CRuby, and with our startup time improvements and the --dev flag, we're able to run those commands faster than any past JRuby version.
